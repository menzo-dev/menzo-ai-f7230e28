            ...
            <div key={post.id} className={`group flex gap-2 ${isOwn ? "flex-row-reverse" : ""}`}>
              <div className="shrink-0">
                {post.profile?.avatar_url ? (
                  <img src={post.profile.avatar_url} alt="" className="h-8 w-8 rounded-full object-cover border border-border/40" />
                ) : (
              ...
              <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${
                ...
                <div className="text-xs font-bold mb-1 opacity-70">{post.profile?.display_name || "مستخدم"}</div>
                <p className="text-sm whitespace-pre-wrap">{post.content}</p>
                {post.image_url && (
                  <img src={post.image_url} alt="" className="mt-2 rounded-xl max-h-60 w-auto" />
                )}
      ...
      {imagePreview && (
        <div className="px-4 py-2 border-t border-border/40 glass-strong">
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setImageFile(null); setImagePreview(null); }} className="text-destructive hover:text-destructive/80">
      ...
      <div className="border-t border-border/40 p-4 glass-strong">
        <div className="max-w-3xl mx-auto flex items-end gap-2">
          ...
          <textarea value={message} onChange={e => setMessage(e.target.value)}
            ...
            rows={1} />
          <Button onClick={sendPost} disabled={!message.trim() || loading} size="icon"
